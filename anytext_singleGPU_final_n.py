...
from t3_dataset import draw_glyph, draw_glyph2, draw_glyph1, get_caption_pos
...
def get_item(data_list, item):
    ...
    if len(texts) > 0:
        ...
        for idx, text in enumerate(item_dict['texts']):
            ...
            # with rotation
            glyphs1 = draw_glyph1(font, text, item_dict['polygons'][idx], scale=2)
...
def process(model, ddim_sampler, item_dict, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, strength, scale, seed, eta):
    with torch.no_grad():
        ...
        for i in info['glyphs']:

            i = i.squeeze()  
            img = transform(i)
            output_file_path = os.path.join(output_dir, "glyphs_image_"+ str(count)+".jpg")
            img.save(output_file_path)
            #img.save('glyphs_' + str(count) + ".png")
            count +=1
            final_image = torch.add(final_image, i)
        glyph_image = transform(final_image)
        #control_image = final_image.resize((512, 512))

        count = 0
        final_image1 = torch.zeros(1024, 1024).to("cuda:0")
        req_list = info['glyphs1']
        for i in req_list:
            i = i.squeeze()
            img = transform(i)
            output_file_path = os.path.join(output_dir, "rotated_glyphs_image_" + str(count)+".jpg")
            img.save(output_file_path)
            count += 1
            final_image1 = torch.add(final_image1, i)
        rotated_glyph_image = transform(final_image1)


        count = 0
        final_image1 = torch.zeros(512, 512).to("cuda:0")
        req_list = info['positions']
        for i in req_list:
            i = i.squeeze()
            img = transform(i)
            output_file_path = os.path.join(output_dir, "pos_image_" + str(count)+".jpg")
            img.save(output_file_path)
            count += 1
            final_image1 = torch.add(final_image1, i)
        pos_image = transform(final_image1)
        ...
        index = 0
        directory = '/home/ubuntu/anytext_v1/AnyText/eval/training_data_v2'
        while True:
        ...
        glyph_image = numpy.array(glyph_image)
        pos_image = numpy.array(pos_image)
        rotated_glyph_image = numpy.array(rotated_glyph_image)
